package com.github.alexwang.kafka.streams.ktable;
...
public class KTableInnerJoinKTable {
    private final static Logger LOG = LoggerFactory.getLogger(KTableInnerJoinKTable.class);
    private final static String APP_ID = "ktable_inner_join_ktable";
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        KTable<String, String> left = builder.table(USER_INFO_TOPIC, Consumed.with(Serdes.String(), Serdes.String())
                .withName("user-source").withOffsetResetPolicy(Topology.AutoOffsetReset.LATEST), Materialized.as("user-name"));

        KTable<String, String> right = builder.table(USER_ADDRESS_TOPIC, Consumed.with(Serdes.String(), Serdes.String())
                .withName("address-source").withOffsetResetPolicy(Topology.AutoOffsetReset.LATEST), Materialized.as("user-address"));

        left.join(right, (userName, address) -> userName + " come from " + address)
                .toStream()
