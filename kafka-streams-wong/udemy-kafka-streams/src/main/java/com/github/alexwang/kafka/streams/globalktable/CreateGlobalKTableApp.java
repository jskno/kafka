package com.github.alexwang.kafka.streams.globalktable;
...
public class CreateGlobalKTableApp {

    private final static Logger LOG = LoggerFactory.getLogger(CreateGlobalKTableApp.class);
    private final static String APP_ID = "global_ktable_creation_app";
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        KStream<String, String> userInfoKS = builder.stream(USER_INFO_TOPIC, Consumed.with(Serdes.String(), Serdes.String())
                .withName("user-source").withOffsetResetPolicy(Topology.AutoOffsetReset.LATEST));

        GlobalKTable<String, String> globalKTable = builder.globalTable(USER_ADDRESS_TOPIC, Consumed.with(Serdes.String(), Serdes.String()).withName("address-source")
                .withOffsetResetPolicy(Topology.AutoOffsetReset.LATEST), Materialized.as("global-table"));

        userInfoKS.join(globalKTable, (k, v) -> k, (username, address) -> String.format("%s come from %s", username, address), Named.as("join"))
