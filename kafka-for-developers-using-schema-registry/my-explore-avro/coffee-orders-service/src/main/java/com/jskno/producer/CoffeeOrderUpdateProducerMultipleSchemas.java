...
public class CoffeeOrderUpdateProducerMultipleSchemas {

    public static final Logger LOGGER = LoggerFactory.getLogger(CoffeeOrderUpdateProducerMultipleSchemas.class);

    public static void main(String[] args) {
        Properties properties = buildProducerProperties();

        KafkaProducer<OrderId, CoffeeOrderUpdate> producer = new KafkaProducer<>(properties);
        OrderId orderId = OrderId.newBuilder().setId(599).build();
        CoffeeOrderUpdate coffeeOrderUpdate = CoffeeOrderMultyplesSchemasUtil.buildCoffeeOrderUpdate(orderId);
        ProducerRecord<OrderId, CoffeeOrderUpdate> record = buildProducerRecord(coffeeOrderUpdate);
    ...
    private static ProducerRecord<OrderId, CoffeeOrderUpdate> buildProducerRecord(CoffeeOrderUpdate coffeeOrder) {
