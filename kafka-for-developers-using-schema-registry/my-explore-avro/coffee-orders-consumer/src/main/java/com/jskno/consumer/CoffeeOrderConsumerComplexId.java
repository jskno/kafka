...
public class CoffeeOrderConsumerComplexId {

    public static final Logger LOGGER = LoggerFactory.getLogger(CoffeeOrderConsumerComplexId.class);
    ...
    public static void main(String[] args) {
        ...
        KafkaConsumer<OrderId, CoffeeOrderComplexId> consumer = new KafkaConsumer<>(properties);

        consumer.subscribe(List.of(CoffeeOrderConstants.COFFEE_ORDER_COMPLEX_ID_TOPIC));

        while (true) {
            ConsumerRecords<OrderId, CoffeeOrderComplexId> records = consumer.poll(Duration.ofMillis(100));
            records.forEach(record -> {
                CoffeeOrderComplexId coffeeOrder = record.value();
    ...
    private static Properties buildConsumerProperties() {
        ...
        properties.put(ConsumerConfig.GROUP_ID_CONFIG, "coffee-orders-consumer-cid");
