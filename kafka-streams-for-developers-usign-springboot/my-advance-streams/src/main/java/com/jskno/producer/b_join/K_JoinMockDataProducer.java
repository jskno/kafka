...
@Slf4j
public class K_JoinMockDataProducer {
    ...
    public static void main(String[] args) {
        ...
        var alphRecords = alphabetMap.entrySet().stream()
            ...
            .toList();
        alphRecords.forEach(K_JoinMockDataProducer::publishMessageSync);
...
//        alphRecords.forEach(JoinMockDataProducer::publishMessageSync);
        ...
        var abbrevRecords = alphabetAbbrevationMap.entrySet().stream()
            .map(entry -> new ProducerRecord<>(ALPHABETS_ABBREVIATIONS, entry.getKey(), entry.getValue()))
            .toList();
        abbrevRecords.forEach(K_JoinMockDataProducer::publishMessageSync);
