package com.jskno.management.orders.streams.producer;

import static com.jskno.management.orders.streams.constants.OrdersConstants.ORDERS;
import static com.jskno.management.orders.streams.producer.ProducerUtil.publishMessageSync;
...
//        publishExpiredRecords(objectMapper);
    ...
    private static void publishFutureRecords(ObjectMapper objectMapper) {
        var offsetDateTime = OffsetDateTime.now().plusDays(1);
    ...
    private static void publishExpiredRecords(ObjectMapper objectMapper) {
        ...
        var offsetDateTime = OffsetDateTime.now().minusDays(1);
    ...
    private static void publishOrdersForGracePeriod(ObjectMapper objectMapper, List<Order> orders) {
        ...
        var offsetDateTime = OffsetDateTime.now().with(modifiedTime);
        ...
        publishRecordsWithDelay(generalOrdersWithCustomTime, offsetDateTime, objectMapper, 18);
    ...
    private static void publishRecordsWithDelay(List<Order> newOrders, OffsetDateTime offsetDateTime, ObjectMapper objectMapper,
        int timeToPublish) {

        var flag = true;
        while (flag) {
            var dateTime = LocalDateTime.now();
            if (dateTime.toLocalTime().getMinute() == offsetDateTime.getMinute()
                && dateTime.toLocalTime().getSecond() == timeToPublish) {
                System.out.print("Publishing the record with delay ");
                publishOrders(objectMapper, newOrders);
                flag = false;
            } else {
                System.out.println(" Current Time is  and the record will be published at the 16th second: " + dateTime);
                System.out.println("Record Date Time : " + offsetDateTime);
            }
        }
    }
    ...
    private static List<Order> buildOrdersForGracePeriod() {
        ...
        var order1 = new Order(12345L, "store_999",
            new BigDecimal("27.00"),
            OrderType.RESTAURANT,
            orderItems,
            OffsetDateTime.parse("2023-01-06T18:50:21")
        );

        var order2 = new Order(54321L, "store_999",
        ...
        var order3 = new Order(54321L, "store_999",
            new BigDecimal("15.00"),
            OrderType.RESTAURANT,
            orderItemsRestaurant,
            OffsetDateTime.parse("2023-01-06T18:50:22")
        );
    ...
    private static List<Order> buildOrders() {
        ...
        var order2 = new Order(100L, "store_1234",
            new BigDecimal("15.00"),
            OrderType.RESTAURANT,
            orderItemsRestaurant,
            OffsetDateTime.now()
        ...
        var order3 = new Order(12345L, "store_4567",
            new BigDecimal("27.00"),
            OrderType.GENERAL,
            orderItems,
            OffsetDateTime.now()
        ...
        var order4 = new Order(100L, "store_4567",
            new BigDecimal("27.00"),
            OrderType.RESTAURANT,
            orderItems,
            OffsetDateTime.now()
    ...
    private static List<Order> buildOrdersToTestGrace() {
        ...
        var order1 = new Order(12345L, "store_1234",
            ...
            OrderType.GENERAL,
            orderItems,
            OffsetDateTime.parse("2024-09-07T13:33:58+02:00")
            //LocalDateTime.now(ZoneId.of("UTC"))
        );
        ...
        var order4 = new Order(12345L, "store_4567",
            new BigDecimal("27.00"),
            OrderType.RESTAURANT,
            orderItems,
            //LocalDateTime.now()
        ...
        orders
            .forEach(order -> {
                try {
                    ...
                    var recordMetaData = publishMessageSync(ORDERS, order.id() + "", ordersJSON);
